...
JSONWriter::JSONWriter(const QString& path) : filePath(path) {
    ...
    if (file.open(QIODevice::ReadOnly)) {
        ...
        if (error.error == QJsonParseError::NoError && doc.isArray()) {
            ...
            qWarning() << "Errore parsing JSON, inizializzazione vuota:" << error.errorString();
        }
    } else {
        qWarning() << "File JSON non trovato. Inizializzazione vuota.";
    }
}
...
void JSONWriter::appendProduct(product::Product* p) {
    p->accept(this);            // riempie currentObject
...
void JSONWriter::save() {
    ...
    if (file.open(QIODevice::WriteOnly | QIODevice::Truncate)) {
        ...
        qWarning() << "Errore scrittura JSON";
    }
}
